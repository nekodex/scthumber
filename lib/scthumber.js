  ...
  var thumber = {
    ...
    thumbnail: function(req, res) {
      ...
      function processFile() {
        ...
        var hiresTime = process.hrtime();
        var timestamp = `${hiresTime[0] * 1e9 + hiresTime[1]}`;

        preset = presets[imagePreset];
        filepath = targetDir + '/' + urlHash + '_' + timestamp + ext;
        modifiedFilePath = targetDir + '/' + urlHash + '_' + timestamp + "-" + imagePreset + ext;
        ...
        var fetcher = http.get(upstreamURL, function (response) {
          if (response.statusCode != 200) {
            ...
            thumber.log(`ERR: ${req.originalUrl} [UPSTREAM: ${response.statusCode}]`.red);
            return;
          }
          ...
          response.on('end', function () {
            ...
            var postModificationFunction = function (err) {
              if (err) {
                thumber.stats.increment('thumb_error');
                res.writeHead(500);
                res.end('error processing image (1)');
                thumber.log(err);
                return;
              }

              var elapsed = Date.now() - request_start;
              thumber.log(`OK: ${req.originalUrl} [${elapsed}ms]`.green);
              ...
              send(req, modifiedFilePath, {maxAge: maxTTLAge})
                .on("error", function(err) {
                  thumber.stats.increment('thumb_error');
                  res.writeHead(500);
                  thumber.log(`ERR: ${req.originalUrl} [error sending image (?)]`.red);
                  console.error(err);
                })
                .on("end", function () {
                  if (tmpCacheTTL == 0) {
                    quietUnlink(filepath);
                    quietUnlink(modifiedFilePath + '-preoptim.jpg');
                    quietUnlink(modifiedFilePath);
                  }
                }).pipe(res);
            };
        ...
        fetcher.on('error', function(err) {
          ...
          res.writeHead(500);
          res.end('error retrieving image');
          thumber.log(`ERR: ${req.originalUrl} [error retrieving image]`.red);
          return;
        });
      }
    },
    ...
    optimize: function(req, res) {
      ...
      function processFile() {
        ...
        filepath = targetDir + '/' + urlHash + '_' + timestamp + ext;
        modifiedFilePath = targetDir + '/' + urlHash + '_' + timestamp + "-optim" + ext;
          ...
          response.on('end', function () {
            ...
            var postModificationFunction = function (err) {
              if (err) {
                thumber.stats.increment('thumb_error');
                res.writeHead(500);
                res.end('error processing image (2)');
                thumber.log(err);
                return;
              }
          ...
          sharp(srcPath)
            ...
            .toFile(dstPath + "-preoptim.jpg", function(err) {
              ...
              fs.createReadStream(dstPath + "-preoptim.jpg")
                .pipe(mozjpeg({quality: preset.quality, args: '-baseline'}))
                .pipe(outStream);
            });

          lockFile.unlock(srcPath + ".lock", function (err) {
            if (err) { thumber.log(err); }
          });
        });
      });
...
function quietUnlink(path, callback) {
...
function optimImage(options, callback) {
  ...
  try {
    lockFile.lock(srcPath + ".lock", function(err) {
      ...
      sharp(srcPath)
        .jpeg({quality: 100})
        .pipe(mozjpeg({quality: 94, args: '-baseline'}))
        .on('error', callback)
        .pipe(outStream);
